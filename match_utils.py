...
def cal_cosine_weighted_question_representation(question_representation, cosine_matrix, normalize=False):
    ...
    if not normalize:
        weighted_question_words = tf.div(weighted_question_words,
                                         tf.expand_dims(tf.add(tf.reduce_sum(cosine_matrix, axis=-1), eps), axis=-1))
...
def cal_full_matching_bak(passage_representation, full_question_representation, decompose_params):
    ...
    return cosine_distance(mp_passage_rep,
                           tf.expand_dims(mp_full_question_rep, axis=1))  # [batch_size, passage_len, decompse_dim]
...
def cal_maxpooling_matching_bak(passage_rep, question_rep, decompose_params):
    ...
    question_rep = multi_perspective_expand_for_3D(question_rep,
                                                   decompose_params)  # [batch_size, question_len, decompse_dim, dim]
    ...
    matching_matrix = cosine_distance(passage_rep,
                                      question_rep)  # [batch_size, passage_len, question_len, decompse_dim]
    ...
    return tf.concat([tf.reduce_max(matching_matrix, axis=2), tf.reduce_mean(matching_matrix, axis=2)], 2)
...
def cal_maxpooling_matching_for_word(passage_rep, question_rep, decompose_params):
    ...
    def singel_instance(x):
        ...
        def single_instance_2(y):
            ...
            return tf.concat([tf.reduce_max(matching_matrix, axis=0), tf.reduce_mean(matching_matrix, axis=0)], 0)
...
def cross_entropy(logits, truth, mask):
    ...
    result = tf.multiply(tf.multiply(truth, log_predictions), mask)
    return tf.multiply(-1.0, tf.reduce_sum(result, -1))
...
def cal_max_question_representation(question_representation, cosine_matrix):
    ...
    question_index = tf.argmax(cosine_matrix, 2)
...
def cal_linear_decomposition_representation(passage_representation, passage_lengths, cosine_matrix, is_training,
    ...
    with tf.variable_scope('lex_decomposition'):
        ...
        if is_training:
            ...
            lex_lstm_cell_fw = tf.contrib.rnn.DropoutWrapper(lex_lstm_cell_fw,
                                                             output_keep_prob=(1 - dropout_rate))
            lex_lstm_cell_bw = tf.contrib.rnn.DropoutWrapper(lex_lstm_cell_bw,
                                                             output_keep_prob=(1 - dropout_rate))
        ...
        if MP_dim > 0:
            if with_full_match:
                ...
                fw_full_decomp_params = tf.get_variable("forward_full_matching_decomp",
                                                        shape=[MP_dim, context_lstm_dim], dtype=tf.float32)
                fw_full_match_rep = cal_full_matching(passage_context_representation_fw, fw_question_full_rep,
                                                      fw_full_decomp_params)
                ...
                bw_full_decomp_params = tf.get_variable("backward_full_matching_decomp",
                                                        shape=[MP_dim, context_lstm_dim], dtype=tf.float32)
                bw_full_match_rep = cal_full_matching(passage_context_representation_bw, bw_question_full_rep,
                                                      bw_full_decomp_params)
            ...
            if with_attentive_match:
                ...
                fw_attentive_decomp_params = tf.get_variable("forward_attentive_matching_decomp",
                                                             shape=[MP_dim, context_lstm_dim], dtype=tf.float32)
                fw_attentive_rep = cal_attentive_matching(passage_context_representation_fw, att_question_fw_contexts,
                ...
                bw_attentive_decomp_params = tf.get_variable("backward_attentive_matching_decomp",
                                                             shape=[MP_dim, context_lstm_dim], dtype=tf.float32)
                bw_attentive_rep = cal_attentive_matching(passage_context_representation_bw, att_question_bw_contexts,
            ...
            if with_max_attentive_match:
                ...
                fw_max_att_decomp_params = tf.get_variable("fw_max_att_decomp_params", shape=[MP_dim, context_lstm_dim],
                                                           dtype=tf.float32)
                fw_max_attentive_rep = cal_attentive_matching(passage_context_representation_fw, max_att_fw,
                                                              fw_max_att_decomp_params)
                ...
                bw_max_att_decomp_params = tf.get_variable("bw_max_att_decomp_params", shape=[MP_dim, context_lstm_dim],
                                                           dtype=tf.float32)
                bw_max_attentive_rep = cal_attentive_matching(passage_context_representation_bw, max_att_bw,
                                                              bw_max_att_decomp_params)
...
def unidirectional_matching(in_question_repres, in_passage_repres, question_lengths, passage_lengths,
                            ...
                            aggregation_lstm_dim, highway_layer_num, with_aggregation_highway, with_lex_decomposition,
    ...
    if MP_dim > 0:
        if with_max_attentive_match:
            ...
            max_att_decomp_params = tf.get_variable("max_att_decomp_params", shape=[MP_dim, input_dim],
                                                    dtype=tf.float32)
    ...
    if with_lex_decomposition:
        lex_decomposition = cal_linear_decomposition_representation(raw_in_passage_repres, passage_lengths,
                                                                    cosine_matrix, is_training,
                                                                    lex_decompsition_dim, dropout_rate)
        ...
        for i in iter(range(context_layer_num)):
            with tf.variable_scope('layer-{}'.format(i)):
                with tf.variable_scope('context_represent'):
                    ...
                    (question_context_representation_fw,
                     question_context_representation_bw), _ = rnn.bidirectional_dynamic_rnn(
                    ...
                    in_question_repres = tf.concat(
                        [question_context_representation_fw, question_context_representation_bw], 2)

                    # passage representation
                    tf.get_variable_scope().reuse_variables()
                    # (passage_context_representation_fw, passage_context_representation_bw), _ = my_rnn.bidirectional_dynamic_rnn(
                    (passage_context_representation_fw,
                     passage_context_representation_bw), _ = rnn.bidirectional_dynamic_rnn(
    ...
    all_question_aware_representatins = tf.concat(all_question_aware_representatins, 2)
    ...
    if with_match_highway:
        with tf.variable_scope("matching_highway"):
            all_question_aware_representatins = multi_highway_layer(all_question_aware_representatins,
                                                                    question_aware_dim, highway_layer_num)
    ...
    with tf.variable_scope('aggregation_layer'):
        ...
        for i in iter(range(aggregation_layer_num)):
            with tf.variable_scope('layer-{}'.format(i)):
                ...
                aggregation_input = tf.concat(cur_aggregation_representation, 2)
    #
    # aggregation_representation = tf.concat(1, aggregation_representation) # [batch_size, aggregation_dim]
    aggregation_representation = tf.concat(aggregation_representation, 1)
    # ======Highway layer======
    ...
    if with_left_match:
        ...
        with tf.name_scope("match_passsage"):
            with tf.variable_scope("MP-Match", reuse=None, initializer=initializer):
                (passage_match_representation, passage_match_dim) = unidirectional_matching(in_question_repres,
                                                                                            in_passage_repres,
                                                                                            question_lengths,
                                                                                            passage_lengths,
                                                                                            question_mask, mask, MP_dim,
                                                                                            input_dim,
                                                                                            with_filter_layer,
                                                                                            context_layer_num,
                                                                                            context_lstm_dim,
    ...
    if with_right_match:
        with tf.name_scope("match_question"):
            with tf.variable_scope("MP-Match", reuse=reuse_match_params, initializer=initializer):
                (question_match_representation, question_match_dim) = unidirectional_matching(in_passage_repres,
                                                                                              in_question_repres,
                                                                                              passage_lengths,
                                                                                              question_lengths, mask,
                                                                                              question_mask, MP_dim,
                                                                                              input_dim,
                                                                                              with_filter_layer,
                                                                                              context_layer_num,
                                                                                              context_lstm_dim,
...
def bilateral_match_func2(in_question_repres, in_passage_repres,
    ...
    cosine_matrix = cal_relevancy_matrix(in_question_repres,
                                         in_passage_repres)  # [batch_size, passage_len, question_len]
    cosine_matrix = mask_relevancy_matrix(cosine_matrix, question_mask, mask)
    ...
    question_aware_dim += 2
    passage_aware_representatins.append(
        tf.reduce_max(cosine_matrix_transpose, axis=2, keep_dims=True))  # [batch_size, question_len, 1]
    passage_aware_representatins.append(
        tf.reduce_mean(cosine_matrix_transpose, axis=2, keep_dims=True))  # [batch_size, question_len, 1]
    passage_aware_dim += 2
    ...
    if MP_dim > 0:
        if with_max_attentive_match:
            ...
            qa_max_att = cal_max_question_representation(in_question_repres,
                                                         cosine_matrix)  # [batch_size, passage_len, dim]
            qa_max_att_decomp_params = tf.get_variable("qa_word_max_att_decomp_params", shape=[MP_dim, input_dim],
                                                       dtype=tf.float32)
            qa_max_attentive_rep = cal_attentive_matching(in_passage_repres, qa_max_att,
                                                          qa_max_att_decomp_params)  # [batch_size, passage_len, decompse_dim]
            question_aware_representatins.append(qa_max_attentive_rep)
            question_aware_dim += MP_dim

            pa_max_att = cal_max_question_representation(in_passage_repres,
                                                         cosine_matrix_transpose)  # [batch_size, question_len, dim]
            pa_max_att_decomp_params = tf.get_variable("pa_word_max_att_decomp_params", shape=[MP_dim, input_dim],
                                                       dtype=tf.float32)
            pa_max_attentive_rep = cal_attentive_matching(in_question_repres, pa_max_att,
                                                          pa_max_att_decomp_params)  # [batch_size, question_len, decompse_dim]
    ...
    question_aware_representatins = tf.concat(question_aware_representatins,
                                              2)  # [batch_size, passage_len, question_aware_dim]
    passage_aware_representatins = tf.concat(passage_aware_representatins, 2)
    if is_training:
    ...
    with tf.variable_scope('aggregation_layer'):
        ...
        for i in iter(range(aggregation_layer_num)):  # support multiple aggregation layer
            with tf.variable_scope('left_layer-{}'.format(i)):
                ...
                qa_aggregation_input = tf.concat(cur_aggregation_representation, 2)
            ...
            with tf.variable_scope('right_layer-{}'.format(i)):
                ...
                pa_aggregation_input = tf.concat(cur_aggregation_representation, 2)
    # aggregation_representation = tf.concat(1, aggregation_representation) # [batch_size, aggregation_dim]
    aggregation_representation = tf.concat(aggregation_representation, 1)
    # ======Highway layer======
    if with_aggregation_highway:
