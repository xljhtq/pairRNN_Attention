...
# import math
class Vocab(object):
    def __init__(self, vec_path=None, dim=100, fileformat='bin', voc=None, word2id=None, word_vecs=None,
    ...
    def fromText_format3(self, vec_path, voc=None):
        ...
        with open("models/word2id.txt", "w") as out_op:
            for word in self.word2id:
                result = word + "\t" + str(self.word2id[word]) + "\n"
                out_op.write(result)
    ...
    def fromBinary_with_voc(self, fname, voc, scale=0.05, stop_num=50):
        ...
        with open(fname, "rb") as f:
            ...
            for idx in iter(range(cur_vocab_size)):
                word = []
                while True:
                    ch = f.read(1)
                    if ch == ' ':
        ...
        self.vocab_size = len(self.word2id)
        self.word_vecs = np.random.uniform(low=-scale, high=scale, size=(self.vocab_size + 1, self.word_dim)).astype(
            'float32')
    ...
    def fromBinary(self, fname, scale=0.05, voc=None):
        ...
        with open(fname, "rb") as f:
            ...
            for idx in iter(range(self.vocab_size)):
                word = []
                while True:
                    ch = f.read(1)
                    if ch == ' ':
    ...
    def getIndex(self, word):
        ...
        if (word in self.word2id):
            return self.word2id.get(word)  # 得到value
        else:
    ...
    def to_character_matrix(self, sentence):
        ...
        for word in re.split('\\s+', sentence):
            cur_seq = []
            for i in iter(range(len(word))):
                cur_char = word[i]
                idx = self.getIndex(cur_char)  # 得到char id
...
def collect_all_ngram(words, n=2):
    ...
    for i in iter(range(len(words) - n)):
...
def collect_char_ngram(word, n=3):
    ...
    if len(word) <= n:
        ...
        for i in iter(range(len(word) - n + 1)):
            cur_word = word[i:i + 3]
