...
class Ranking(object):
    def __init__(self, max_len_left, max_len_right,
        ...
        self.input_y = tf.placeholder(tf.float32, [None, 2], name="input_label")
        self.dropout_keep_prob = tf.placeholder(tf.float32, name="dropout_keep_prob")
        print(self.input_y.name)
        ...
        ######## Section 1:  Create a convolution + maxpool layer for each filter size
        pooled_outputs_left = []
        ...
        ######### Section 2: Combine all the pooled features
        num_filters_total = num_filters * len(filter_sizes)  # 64*2
        self.pooled_outputs_left = pooled_outputs_left
        self.m = tf.concat(axis=3, values=pooled_outputs_left)
        ...
        ######### Section 3: Compute similarity
        with tf.name_scope("similarity"):
            W = tf.get_variable("W",
                                shape=[num_filters_total, num_filters_total],
                                initializer=tf.contrib.layers.xavier_initializer())
            self.transform_left = tf.matmul(self.h_pool_left, W)
            self.sims = tf.reduce_sum(tf.multiply(self.transform_left, self.h_pool_right), 1, keep_dims=True)  # 点乘
            print (self.sims)
        ...
        with tf.name_scope("output"):
            ...
            self.predictions = tf.argmax(self.scores, 1, name="predictions")  # axis表示比较范围

        ######### Section 8: CalculateMean cross-entropy loss
